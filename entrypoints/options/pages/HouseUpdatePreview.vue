  ...
  <div  v-if="data" :class="cn('flex flex-col space-y-6', props.class)">
    ...
    <div v-if="data.miss.length > 0" class="flex flex-col space-y-2">
      <ul class="flex flex-col gap-3">
        <li v-for="(item,index) in data.miss" :key="item.hid"
    ...
    <div v-if="data.sold.length > 0" class="flex flex-col space-y-2">
      <ul class="flex flex-col gap-3">
        <li v-for="(item,index) in data.sold" :key="item.hid"
            class="text-sm  flex flex-row items-center gap-3"
    ...
    <div v-if="data.normal.length > 0" class="flex flex-col space-y-2">
      <div v-for="(item,index) in data.normal" :key="item.hid" class="flex flex-row gap-2 items-center"
        ...
        <p v-if="item.newPrice" class="text-sm text-nowrap text-green-500 bg-neutral-100 border rounded hover:cursor-help" title="之前状态不是[在售]">新状态: 在售</p>
    ...
    <div v-if="!isUpdateDone">
      <Button @click="mutate" :disabled="status!=='idle'" >
        <div v-if="status==='pending'" class="w-4 h-4 rounded-full animate-spin  border-2 border-t-transparent border-green-500"></div>
        确认数据
      </Button>
    </div>
  </div>
