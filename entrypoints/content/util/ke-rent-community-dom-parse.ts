...
export async function parseAllOfKeRentCommunity(): Promise<any> {
  const name = document.querySelector(".sec1")?.textContent;
  ...
  console.log("name", name, totalCount, communityUrl, city, cid, pageNo);
  document
    ...
    .forEach((element) => {
      ...
      console.log("name", name, desc, price);
    });
  return {};
}
