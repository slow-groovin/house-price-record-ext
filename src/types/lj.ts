...
export class HouseTask implements HouseItem {
  ...
  static fromHouseTask(variable: HouseTask) {
    ...
    task.accessRecord = Object.assign(
      new AccessRecord(),
      variable.accessRecord
    );
 ...
 * 成交页面的列表项Item
 */
export interface HouseSoldItem {
  hid: string;
  soldAt: number;
}
...
export enum CommunityTaskStatus {
  running = 1,
  pause = 2,
}
