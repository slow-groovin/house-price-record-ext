...
import QuickLoginDesc from "@/entrypoints/options/components/description/QuickLoginDesc.vue";
    ...
    <div class="flex flex-wrap gap-4">
      <div class="p-2 border rounded w-fit ">
        ...
        <div class="inline-flex gap-1 flex-wrap text-wrap break-words">
          请先打开任意小区在售二房房列表页面进行测试
          <a v-if="lastRunningTask" :href="genCommunityPageUrl(lastRunningTask.city, lastRunningTask.cid, 1)"
            class="link" target="_blank" rel="noreferrer">打开示例页面</a>
          , 确保
          <InfoHover>
            <template #trigger>
            ...
            <template #default>
              <div class="relative">
                <div class="absolute top text-3xl font-bold bg-white">
                  可以打开并正常访问这种列表页面
                </div>
                <img src="/desc/community-login.jpg" alt="" class="w-[48rem]">
              </div>
            </template>
          </InfoHover>
        </div>
