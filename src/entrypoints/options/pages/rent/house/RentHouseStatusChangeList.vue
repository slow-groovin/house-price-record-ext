...
import { RentHouseStatusChange } from "@/types/rent";
...
const sortCondition = ref<SortState<RentHouseStatusChange>>({})
...
data
 */
const data = ref<RentHouseStatusChange[]>([])
...
async function queryData(_pageIndex?: number, _pageSize?: number) {
  ...
  const rs = await RentDao.from('ke').findManyChanges('status', pageConditon, queryCondition.value, sortCondition.value)
  data.value = rs.data
...
<template>
  ...
  <div class="mb-5 rounded p-2 border">
    查询条件:
    <HouseStatusChangeQueryDock v-model="queryCondition" @update="onApplyQueryCondition" type="rent" />
  </div>

  <div class="mb-5 rounded p-2 border">
    排序:
    <GenericSortDock v-model="sortCondition" :fields="['at', 'cid']" :field-text-map="frequentFieldZhMap"
