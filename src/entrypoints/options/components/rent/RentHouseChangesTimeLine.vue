...
import { RentHouse, RentHousePriceChange, RentHouseStatusChange } from "@/types/rent";
...
const { data, rowCount, isShowDetail, type, class: classNames } = defineProps<{
  data: (RentHousePriceChange | RentHouseStatusChange)[],
  rowCount: number,
...
data
 */
const hMap = ref<Record<string, RentHouse>>({})


async function queryDetailData() {
  console.log('[RentHouseChangesTimeLine.vue] queryDetailData()')
  //changes rid cid
  const ridList = Array.from(new Set(data.map(c => c.rid)))


  const houseTasks = await RentDao.from('ke').findHousesByRid(ridList)
  houseTasks.forEach(h => hMap.value[h.rid] = h)
}
...
function deleteChange(index: number) {
  ...
  RentDao.from('ke').deleteChangeById(change.id!).then(() => {
    ...
    <TransitionGroup name="list" tag="div">
      <template v-for="(change, index) in data" :key="change.id">
        <TimelineItem v-if="!deletedIds.includes(change.id)" class="max-w-full" :spacing="80" :color="'bg-green-300'">
          <div class="flex flex-col  gap-2  mb-6 overflow-x-ridden  ">
            <div class="flex flex-row gap-2 items-center ">
              ...
              <div v-if="isShowDetail" class="text-nowrap overflow-ridden overflow-ellipsis"
                ...
                {{ hMap[change.rid]?.name }} {{ hMap[change.rid]?.desc }}
              </div>
