...
export const ljMetricRules: any[] = [
...

  {
    ...
    action_desc:
      "阻止lianjia统计用户浏览网页的行为信息(用以参与决策制定买方价格底线)",
...

  {
    ...
    action_desc:
      "阻止百度相关的所有营销/行为分析/功能请求: 营销分析,位置上报,用户行为分析,百度地图内嵌等等",
...
const ljRulesWithoutDesc = ljMetricRules.map((rule) =>
  omit(rule, ["action_desc", "effect_desc", "default_on", "must_on"])
);

//只记录 开启的
const getBlockSettings = async () =>
  await storage.getItem<Record<number, boolean>>("local:block-setting", {
    fallback: ljMetricRules
...
export async function updateLjRulesById(
  ...
  await browser.declarativeNetRequest.updateDynamicRules({
    ...
    //@ts-ignore
    addRules: addRuleIds.map((id) =>
      ljRulesWithoutDesc.find((rule) => rule.id == id)
    ),
  });
}
