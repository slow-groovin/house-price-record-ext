...
export async function updateBatchHouseWithPreview(
  preview?: HousesUpdatePreview
) {
  if (!preview) {
    alert("没有数据!");
    return;
 ...
 */
export async function updateOneNormal(
  ...
  const { updateChanges, commonFieldChanges, newPrice, newStatus } =
    houseNormal;
  let changes = {
    ...updateChanges,
    lastRunningAt: at,
...
export async function updateOneMiss(
  item: HouseUpdateBase,
  taskInDb: HouseTask,
  at: number
) {
  const task = await db.houseTasks.where("hid").equals(item.hid).first();
...
export async function updateOneSold(
  item: HouseSold,
  taskInDb: HouseTask,
  at: number
) {
